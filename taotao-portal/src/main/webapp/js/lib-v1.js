...
var category = {OBJ: $("#_JD_ALLSORT"),
		...
		URL_Serv:"http://localhost:8081/taotao-rest/rest/itemcat/list?callback=category.getDataService",// 因为下面请求json的时候已经为category.json加上了category.getDataService的外包装了
																										// 所以不用加callback=xxx
		// URL_Serv: "category.json",
		// URL_Serv:
		// "http://localhost:8081/taotao-rest/category.json",//这里的的url会用作下面的添加回调函数category.getDataService的前面的部分？
       ...
       /*
    	...
    	// 使用jsonp来实现跨域请求
    	// jsonp其实就是一个跨域解决方案 js跨域请求数据是不可以的,但是js跨域请求js脚本是可以的,可以将数据封装成一个json语句
		// ,做一个方法的调用 跨域请求js脚本可以得到此脚本 得到js脚本之后立即执行 可以吧数据作为参数传递到方法中 就可以获得数据
		// 从而解决跨域问题
